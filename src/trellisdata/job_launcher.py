...
class TaskConfiguration(yaml.YAMLObject):
    yaml_tag = u'!TaskConfiguration'
    def __init__(
                 self, 
                 name,
                 inputs,
...
class JobRequest():
    ...
    def __init__(self, task, nodes):
        ...
        self.job_id, self.trunc_nodes_hash = JobRequest._make_unique_job_id(self.nodes)
    ...
    def _make_unique_job_id(nodes):
        ...
        datetime_stamp = utils._get_datetime_stamp()
        task_id = f"{datetime_stamp}-{trunc_nodes_hash}"
        return(task_id, trunc_nodes_hash)

    @staticmethod
    def _get_input_output_values(task, argument_type):
        ...
        task_fields = task.dsub[argument_type]
    ...
    def get_job_sources(self, query_response, trellis_config):
        ...
        for source in self.task.sources:
            if source['graph_status'] == 'independent':
            ...
            elif source['graph_status'] == 'relationship':
    ...
    def configure_dsub_job(self):
        ...
        dsub_args = self._create_dsub_args()
        return dsub_args